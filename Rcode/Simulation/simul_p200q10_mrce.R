...
for (j in 1:length(lams)) {
	...
	for (i in 1:q){
		...
		X = NULL
		if (i ==1){
			G[chlist[[i]],chlist[[i]]] = glasso(s=cov(Y),rho=lam)$wi
		}else{
			X = dat$Y[,palist[[i]]]
			fit=mrce(Y=Y, X=X, lam1=lam, lam2=lam, method="single")
            G[as.matrix(expand.grid(chlist[[i]],palist[[i]]))] = t(fit$Bhat)
			G[chlist[[i]],chlist[[i]]] = fit$omega
		}
	}
	fitlist[[j]] = G
}

save(dat,sim,fitlist,file=paste("../simul_results/mrce_fitlist_p",p,"q",q,"pE",pE,"n",n,"r",r,".rda",sep=""))
